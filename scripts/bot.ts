...
const main = async () => {
  ...
  //Contracts for forked network testing
  ...
  uFactory.on("PairCreated", async () => {
    ...
    // Verify token0 & token1 address, and fetch the address of the new token
    let path: string[] = [];
    ...
    try {
      const amountIn = ethers.utils.formatEther(
        ethers.utils.parseEther(AMOUNT)
      );
      const amounts = await uRouter.methods
      ...
      await WETH.methods
        .approve(uRouter._address, amountIn)
        .send({ from: accounts.sniper });
      ...
      const gas = await uRouter.methods
        ...
        .estimateGas({ from: accounts.sniper });
      await uRouter.methods
        ...
        .send({ from: accounts.sniper, gas: gas });
      ...
      const tokenBalance = await token.methods
        .balanceOf(accounts.sniper)
        .call();

      console.log(
        `Successfully swapped ${AMOUNT} WETH for ${ethers.utils.parseEther(
